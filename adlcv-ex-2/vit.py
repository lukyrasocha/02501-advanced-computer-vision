...
class Attention(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        out = self.o_projection(out)

        if return_attention:
            # shape of attention is (bsz * num_heads, seq_len, seq_len)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        if return_attention:
            attention_out, attention_map = self.attention(x, return_attention=True)
        else:
            attention_out = self.attention(x, return_attention=False)
            attention_map = None
        ...
        # Return attention_map so the next level can aggregate it
...
class ViT(nn.Module):
    ...
    def forward(self, img, return_all_attentions=False):
        tokens = self.to_patch_embedding(img)
        bsz, num_patches, embed_dim = tokens.size()

        if self.pool == 'cls':
            cls_tokens = repeat(self.cls_token, '1 1 e -> b 1 e', b=bsz)
            tokens = torch.cat([cls_tokens, tokens], dim=1)
            num_patches += 1

        positions = self.positional_embedding.to(img.device, dtype=img.dtype)
        if self.pos_enc == 'fixed' and self.pool == 'cls':
            # cat zero embedding for cls token position
            positions = torch.cat([
                torch.zeros(1, embed_dim, device=img.device, dtype=img.dtype),
                positions
            ], dim=0)

        x = tokens + positions  # shape: (bsz, num_patches, embed_dim)
        x = self.dropout(x)

        all_attentions = []  # to store attention from each layer
        ...
        if return_all_attentions:
            # all_attentions will be a list of shape [num_layers] each of shape (bsz * num_heads, seq_len, seq_len)
