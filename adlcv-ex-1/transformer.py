...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # attention_logits shape => (B*h, S, S)
        attention_logits = torch.bmm(queries, keys.transpose(1, 2))

        attention_logits = attention_logits * self.scale
        attention = F.softmax(attention_logits, dim=-1)    # softmax over last dim => (B*h, S, S)
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_seq_len=512):
        ...
        div_term = torch.arange(0., embed_dim, 2) # [0, 2, 4, 6, 8, ..., embed_dim]
...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pool == 'cls':
            ...
            cls_tokens = repeat(self.cls_token, '1 s d -> b s d', b=batch_size)
            # concat on dim=1 => shape: (B, S+1, E)
        ...
        ####################### insert code here #######################
            ...
            x = x[:, 0, :]              # => (B, E) take the first token only
        ################################################################
